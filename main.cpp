// main.cpp
//
// LiDAR recorder service (Hesai XT32M2X-style UDP stream):
...
// - Writes worker_lifecycle.json (updated on shutdown)
// - Logs all API commands to api_commands.jsonl (append-only)
//
// Build:
//   sudo apt-get install -y build-essential cmake libpcap-dev
//   (vendor httplib.h next to this file: https://github.com/yhirose/cpp-httplib)
//
// Run:
//   ./lidar_recorder --bind 127.0.0.1 --port 8080 --state_dir /path/to/state
//
// Start session:
...
//   }'
//
// Restart session (stop + start):
//   curl -s -X POST http://127.0.0.1:8080/restart -H 'Content-Type: application/json' -d '{ ... }'
